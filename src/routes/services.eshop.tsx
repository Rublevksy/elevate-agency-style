    ...
    <>
      <section className="page-top pb-20 md:pb-28 relative overflow-hidden">
        ...
        <div className="container-luxe relative grid lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7">
            <Link to="/services" className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-emerald-300/80 hover:text-foreground transition-colors mb-12">
              <ArrowLeft className="h-3.5 w-3.5" /> Všechny služby
            </Link>
            <p className="text-xs font-mono uppercase tracking-[0.3em] text-emerald-300 mb-6">E-commerce performance</p>
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              <p className="text-sm text-muted-foreground"><span className="text-xs uppercase tracking-widest text-emerald-300 mr-2">Cena</span><span className="text-foreground font-bold">od 25 000 Kč</span></p>
            </div>
          </div>
          <div className="lg:col-span-5">
            <div className="rounded-xl border border-border bg-background/80 backdrop-blur-sm overflow-hidden shadow-2xl">
              <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-surface/60">
                <span className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-muted-foreground"><ShoppingCart className="h-4 w-4 text-emerald-300" /> Live katalog</span>
                <span className="text-[10px] font-mono text-emerald-300">+38 % CR</span>
              </div>
              <div className="grid grid-cols-2 gap-4 p-5">
                {products.map((p) => (
                  <div key={p.sku} className="rounded-lg border border-border bg-surface/40 overflow-hidden">
                    <div className="aspect-square bg-gradient-to-br from-emerald-400/30 via-primary/10 to-transparent relative">
                      <span className="absolute left-3 top-3 text-[9px] font-mono text-emerald-300 bg-background/70 px-2 py-1 rounded">{p.tag}</span>
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-sm font-bold text-emerald-300 mt-1">{p.price}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-24 md:py-32 border-t border-border">
        <div className="container-luxe grid lg:grid-cols-12 gap-14">
          <div className="lg:col-span-5">
            <p className="text-xs uppercase tracking-[0.25em] text-emerald-300 mb-5">Co děláme</p>
          ...
          <ul className="lg:col-span-7 grid sm:grid-cols-2 gap-4">
            {["návrh struktury e-shopu", "UX optimalizace", "napojení plateb", "optimalizace produktů"].map((item) => (
              <li key={item} className="rounded-xl border border-border bg-surface/40 p-6 flex gap-4"><Check className="h-5 w-5 text-emerald-300 shrink-0" /><span className="text-foreground">{item}</span></li>
            ))}
          </ul>
        </div>
      </section>
      ...
      <section className="py-24 md:py-32 border-t border-border bg-surface/30">
        <div className="container-luxe">
          <p className="text-xs uppercase tracking-[0.25em] text-emerald-300 mb-12">Proces</p>
          <ol className="grid md:grid-cols-4 gap-0 border-y border-border">
            {["Analýza", "Návrh", "Vývoj", "Spuštění"].map((step, i) => (
              <li key={step} className="p-8 md:border-r md:last:border-r-0 border-border">
                <div className="text-6xl font-bold text-emerald-300/80 font-mono mb-4">{i + 1}</div>
      ...
      <section className="py-24 md:py-32 border-t border-border">
        <div className="container-luxe grid md:grid-cols-2 gap-6">
          {[{ Icon: TrendingUp, n: "+ vyšší konverze", l: "Lepší produktové stránky, košík a objednávkový tok." }, { Icon: BarChart3, n: "+ více objednávek", l: "E-shop navržený podle nákupního chování zákazníků." }].map(({ Icon, n, l }) => (
            <div key={n} className="rounded-xl border border-border bg-gradient-to-br from-emerald-400/10 to-transparent p-10">
              <Icon className="h-7 w-7 text-emerald-300 mb-8" />
      ...
      <section className="py-24 md:py-32 border-t border-border bg-surface/30">
        <div className="container-luxe text-center">
          <p className="text-xs uppercase tracking-widest text-emerald-300 mb-4">Pracujeme s omezeným počtem klientů</p>
