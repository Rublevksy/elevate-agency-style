...
const TIP: Record<Lang, string> = {
  CZ: "Napište nám na WhatsApp",
  EN: "Message us on WhatsApp",
  RU: "Напишите нам в WhatsApp",
  UA: "Напишіть нам у WhatsApp",
};

// Placeholder — owner will update
...
export function WhatsAppFab() {
  ...
  return (
    <a
      ...
      aria-label={TIP[lang]}
      className="fixed z-40 bottom-24 right-4 md:bottom-6 md:right-6 inline-flex items-center gap-2"
    >
      {hover && (
        <span className="hidden md:inline-block rounded-md bg-foreground text-background px-3 py-1.5 text-xs shadow-lg whitespace-nowrap">
          {TIP[lang]}
        </span>
      )}
      <span className="grid place-items-center h-14 w-14 rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_-8px_rgba(37,211,102,0.6)] hover:scale-105 transition-transform">
