...
export function Lighting({ stage }: { stage: React.RefObject<number> }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[18, 26, 14]} intensity={1.7} color="#dfe8f5" />
      ...
      <Environment resolution={256} frames={1} background={false}>
        ...
        <Lightformer form="rect" intensity={3.2} position={[0, 16, 4]} rotation={[-Math.PI / 2, 0, 0]} scale={[40, 30, 1]} color="#eaf1fb" />
        <Lightformer form="rect" intensity={1.2} position={[0, 6, 22]} rotation={[0, 0, 0]} scale={[36, 20, 1]} color="#c3d2e6" />
        {/* narrow strips: these are what draw the machined edge highlights */}
        <Lightformer form="rect" intensity={4.5} position={[-16, 5, 6]} rotation={[0, Math.PI / 2.2, 0]} scale={[18, 1.6, 1]} color="#dce7f6" />
        <Lightformer form="rect" intensity={3.4} position={[16, 4, -5]} rotation={[0, -Math.PI / 2.2, 0]} scale={[18, 1.3, 1]} color="#a8bedd" />
        <Lightformer form="circle" intensity={1.3} position={[0, -6, 10]} scale={10} color="#33415a" />
      </Environment>

      <spotLight position={[0, 30, 20]} angle={0.42} penumbra={1} intensity={120} color="#e8f0fb" distance={60} />
